...
class Solution{
   ...
   int index=0;
   int sum=0;
   funct(index,sum,arr,N,list);
   
   Collections.sort(list);
   return list;
    }
    
    private void funct(int index, int sum, ArrayList<Integer> arr,int N,ArrayList<Integer> list){
        if(index==N){
            list.add(sum);
            return;
        }
        funct(index+1,sum+arr.get(index),arr,N,list );
        funct(index+1,sum,arr,N,list );
